...
ChooseClusterResolutionDownsample <- function(input.srobj, res.low = .1, res.high = 1.2, by = 0.02, n.pcs = 30, bias = "over", figdir) {
	...
	#srobj.tmp <- RunPCA(srobj.tmp)
	#srobj.tmp <- RunUMAP(srobj.tmp, reduction = "pca", dims = 1:30)
	#srobj.tmp <- FindNeighbors(srobj.tmp, reduction = "pca", dims = 1:30)
	...
	snn <- ifelse(DefaultAssay(srobj.tmp) == "integrated", "integrated_snn_res.", "SCT_snn_res.")
